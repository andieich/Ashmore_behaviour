---
title: "Model bouts & bites Absent <> After"
format: html
editor: visual
---
...
```{r}
dat_bites_ab_af <- readRDS(here("data/clean/dat_absent_after.rds")) %>% 
  filter(bite>0) 

dat_bites_ab_af <- dat_bites_ab_af %>% 
...
```{r}
dat_bites_ab_af_S <- dat_bites_ab_af %>% 
...
fit_bites_ab_af_S <- brm(
  ...
  file = here("models/fit_bites_ab_af_S"),
  threads = threading(2, static = T),
  backend = "cmdstanr"
)
  
summary(fit_bites_ab_af_S)#ok
pp_check(fit_bites_ab_af_S, ndraws = 100)
bayes_R2(fit_bites_ab_af_S)#0.5493614
```
