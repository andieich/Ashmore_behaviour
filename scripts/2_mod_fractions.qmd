...
```{r}
dat_frac_ab_afW %>% 
  select(size_class, year, video, treatment) %>% 
  unique() %>% 
  group_by(size_class, year, treatment) %>% 
  summarise(n_vid = n()) %>% 
  nice_table()
...
epred_frac_ab_af_SM_S <- epred_frac_ab_af_SM %>%
  ...
  summarise(
    ...
    l_95 = quantile(.epred, 0.025, na.rm = T),
    u_95 = quantile(.epred, 0.975, na.rm = T),
  ) %>%
  mutate(
    type = dplyr::recode(
      .category,
      "frac_feedingT" = "Feeding",
      "frac_circ_invT" = "Investigating & Circling" ,
...
plot_frac <- ggplot(
  data = dat_frac_ab_afL %>%
    filter(size_class != ">100") %>%
    mutate(
      type = dplyr::recode(
        type,
        "feeding_frac" = "Feeding",
        "investigating_circling_frac" = "Investigating & Circling" ,
        "passing_frac" = "Passing"
      )
    ) ,
  aes(x = factor(year), col = treatment)
) +
  geom_point(
    aes(y = perc),
  ...
  scale_colour_manual(values = cols_treat) +
  guides(color = guide_legend(override.aes = list(alpha = 1), title = "Treatment")) +
  facet_grid(size_class ~ type, scales = "free") +
  labs(x = NULL, y = "Fraction") +
  theme_clean(); plot_frac


ggsave(filename = "fractions.pdf",
       plot = plot_frac, 
       path = here("plots"),
...
```{r}
fractions_tenary <- epred_frac_ab_af_SM %>%
  ...
  theme_tern_clean(); fractions_tenary


ggsave(filename = "fractions_tenary.pdf",
       plot = fractions_tenary, 
       path = here("plots"),
