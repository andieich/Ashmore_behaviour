---
title: "Model behaviour fractions Before <> After"
author: "Andreas Eich"
format: html
editor: visual
...
```{r}
dat_frac_be_af <- readRDS(here("data/clean/dat_before_after.rds"))
```
...
```{r}
dat_frac_be_afW <- dat_frac_be_af %>%
...
dat_frac_be_afL <- dat_frac_be_afW %>%
...
```{r}
dat_model_frac_be_af <- dat_frac_be_afW %>% 
...
```{r}
dat_model_frac_be_af <- dat_model_frac_be_af %>% 
...
```{r}
dat_model_frac_be_af_SM <- dat_model_frac_be_af %>% 
  filter(size_class != ">100") %>% 
  droplevels()

dat_model_frac_be_af_L <- dat_model_frac_be_af %>% 
  filter(size_class == ">100") %>% 
  droplevels()
```
...
```{r}
fit_frac_be_af_SM <- brm(bf(cbind(frac_circ_invT, frac_passingT, frac_feedingT) ~ 
           ...
           file = here("models/fit_frac_be_af_SM"),
           backend = "cmdstanr")
```

```{r}
summary(fit_frac_be_af_SM)#1 divergent transition bit Rhat OK
#plot(fit_frac_be_af_SM) # ok
```

Get R2 value per category

```{r}
#dirichlet_r2(fit_frac_be_af_SM)# takes some time
```

rac_circ_invT: 0.21936006\
frac_passingT: 0.23747565\
frac_feedingT: 0.07854487

Get expected values

```{r}
epred_frac_be_af_SM <-  epred_draws(
  fit_frac_be_af_SM,
  newdata = expand_grid(
    year = dat_model_frac_be_af_SM$year %>% unique(),
    treatment = dat_model_frac_be_af_SM$treatment %>% unique(),
    size_class = c("<50", "50-100"),
    habitat = dat_model_frac_be_af_SM$habitat %>% unique(),
    log_depth = mean(dat_model_frac_be_af_SM$depth) %>% 
      log(),
    log_n_frame_total = mean(dat_model_frac_be_af_SM$n_frame_total) %>%
      log()
  ),
  re_formula = NA
)
...
epred_frac_be_af_SM_S <- epred_frac_be_af_SM %>%
...
ggplot(data = dat_frac_be_afL %>%
  ...
  geom_point(aes(y = perc),
             position = position_jitterdodge(jitter.width = .3, 
                                             dodge.width = .5), 
             alpha = .2)+
  geom_errorbar(data = epred_frac_be_af_SM_S, 
             aes(ymin = l_89, ymax = u_89),
             position = position_dodge(width = .5), 
             width = 0,
             size = 2)+
  geom_point(data = epred_frac_be_af_SM_S, 
             aes(y = median),
             position = position_dodge(width = .5), 
             size = 2.5,
             shape = 21,
...
```{r}
epred_frac_be_af_SM_effect <- epred_frac_be_af_SM %>%
  ungroup() %>% 
...
```{r}
epred_frac_be_af_SM_depth <-  epred_draws(
  fit_frac_be_af_SM,
  newdata = expand_grid(
    year = dat_model_frac_be_af_SM$year %>% unique(),
    treatment = dat_model_frac_be_af_SM$treatment %>% unique(),
    ...
    habitat = dat_model_frac_be_af_SM$habitat %>% unique(),
    log_depth = seq(
      min(dat_model_frac_be_af_SM$log_depth),
      max(dat_model_frac_be_af_SM$log_depth),
      length = 100
    ),
    log_n_frame_total = mean(dat_model_frac_be_af_SM$n_frame_total) %>%
      log()
...
epred_frac_be_af_SM_depthS <- epred_frac_be_af_SM_depth %>% 
...
epred_frac_be_af_SM_depthS %>%
  ...
  geom_line(aes(y = median*100, col = .category), linewidth = 1, show.legend = F)+
   geom_point(data = dat_frac_be_afL %>% 
               filter(size_class != ">100") %>% 
...
```{r}
epred_frac_be_af_SM_depth_ntot <-  epred_draws(
  fit_frac_be_af_SM,
  newdata = expand_grid(
    year = dat_model_frac_be_af_SM$year %>% unique(),
    treatment = dat_model_frac_be_af_SM$treatment %>% unique(),
    ...
    log_n_frame_total = seq(
      min(dat_model_frac_be_af_SM$log_n_frame_total),
      max(dat_model_frac_be_af_SM$log_n_frame_total),
      length = 100
    )),
  re_formula = NA
)
...
epred_frac_be_af_SM_depth_ntotS <- epred_frac_be_af_SM_depth_ntot %>% 
...
epred_frac_be_af_SM_depth_ntotS %>%
...
fit_frac_be_af_L <- brm(
  ...
  file_refit = "on_change",
  threads = threading(2, static = T),
  file = here("models/fit_frac_be_af_L"),
  backend = "cmdstanr"
)
```

```{r}
summary(fit_frac_be_af_L)# OK
#plot(fit_frac_be_af_L) # OK
```

```{r}
#dirichlet_r2(fit_frac_be_af_L)
```

frac_circ_invT 0.10352529\
frac_passingT 0.14085995\
frac_feedingT 0.03711858

Get expected values

```{r}
epred_frac_be_af_L <-  epred_draws(
  fit_frac_be_af_L,
  newdata = expand_grid(
    year = dat_model_frac_be_af_L$year %>% unique(),
    treatment = dat_model_frac_be_af_L$treatment %>% unique(),
    habitat = dat_model_frac_be_af_L$habitat %>% unique(),
    log_depth = mean(dat_model_frac_be_af_L$log_depth) %>% 
      log(),
    log_n_frame_total = mean(dat_model_frac_be_af_L$n_frame_total) %>%
      log()
  ),
  re_formula = NA
) %>%
...
```{r}
epred_frac_be_af_L_effect <- epred_frac_be_af_L %>%
  ungroup() %>%
  dplyr::select(year,
                treatment,
                .draw,
