---
title: "Model behaviour Percent Before <> After"
author: "Andreas Eich"
format: html
editor: visual
...
```{r}
dat <- readRDS(here("data/clean/dat_before_after.rds"))
```

-   Ccombine circling and investigating, calculate relative time of behavior types,
...
```{r}
fit_perc_SM <- brm(bf(cbind(frac_circ_invT, frac_passingT, frac_feedingT) ~ 
           ...
           file = here("models/fit_perc_SM"),
           backend = "cmdstanr")
```

```{r}
summary(fit_perc_SM)#1 divergent transition bit Rhat OK
#plot(fit_perc_SM) # ok
```
...
```{r}
epred_perc_SM <-  epred_draws(
  fit_perc_SM,
  newdata = expand_grid(year = dat_model$year %>% unique(),
                        treatment = dat_model$treatment %>% unique(),
                        size_class = c("<50", "50-100"),
                        habitat = dat_model$habitat %>% unique(),
                        log_depth = dat_model$log_depth %>% mean(),
                        log_n_frame_total = mean(dat_model$n_frame_total) %>% 
                          log()),
  re_formula = NA)
```
...
```{r}
epred_perc_effect_SM <- epred_perc_SM %>%
  ungroup() %>% 
...
```{r}
epred_perc_SM_depth <-  epred_draws(
  fit_perc_SM,
  newdata = expand_grid(year = dat_model$year %>% unique(),
                        treatment = dat_model$treatment %>% unique(),
                        size_class = c("<50", "50-100"),
                        habitat = dat_model$habitat %>% unique(),
                        log_depth = seq(min(dat_model$depth),
                                        max(dat_model$depth),
                                        length = 100) %>% 
                          log(),
                        log_n_frame_total = mean(dat_model$n_frame_total) %>% 
...
epred_perc_SM_depthS <- epred_perc_SM_depth %>% 
...
```{r}
epred_perc_SM_n_tot <-  epred_draws(
  fit_perc_SM,
  newdata = expand_grid(year = dat_model$year %>% unique(),
                        treatment = dat_model$treatment %>% unique(),
                        size_class = c("<50", "50-100"),
                        habitat = dat_model$habitat %>% unique(),
                        log_depth = mean(dat_model$depth) %>% 
                          log(),
                        log_n_frame_total = seq(min(dat_model$n_frame_total),
                                        max(dat_model$n_frame_total),
                                        length = 100) %>% 
                          log()),
  re_formula = NA)


epred_perc_SM_n_totS <- epred_perc_SM_n_tot %>% 
...
fit_perc_L <- brm(
  ...
  file_refit = "on_change",
  threads = threading(2, static = T),
  file = here("models/fit_perc_L"),
  backend = "cmdstanr"
)
```

```{r}
summary(fit_perc_L)# 5 divergent transitions, Rhat OK
#plot(fit_perc_L) # OK
```
...
```{r}
epred_perc_effect_L <- epred_perc_L %>%
  ungroup() %>%
  dplyr::select(year,
                treatment,
                .draw,
