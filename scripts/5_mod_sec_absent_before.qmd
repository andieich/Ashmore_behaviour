---
title: "Model behaviour seconds Absent <> Before"
author: "Andreas Eich"
format: html
editor: visual
...
```{r}
dat_sec_ab_be <- readRDS(here("data/clean/dat_absent_before.rds"))
```

Calculate total observation time

```{r}
dat_sec_ab_be <- dat_sec_ab_be %>%
...
```{r}
dat_sec_ab_be_SM <- dat_sec_ab_be %>% 
  filter(size_class != ">100") %>% 
  droplevels()

dat_sec_ab_be_L <- dat_sec_ab_be %>% 
  filter(size_class == ">100") %>% 
  droplevels()
```
...
fit_sec_ab_be_SM <- brm(
  ...
  file = here("models/fit_sec_ab_be_SM"),
  threads = threading(2, static = T),
  backend = "cmdstanr"
)
  
summary(fit_sec_ab_be_SM)# 6 divergent transitions
pp_check(fit_sec_ab_be_SM, ndraws = 100)
bayes_R2(fit_sec_ab_be_SM)#0.51
```

Get predicted draws

```{r}
pred_sec_ab_be_SM <-  predicted_draws(
  fit_sec_ab_be_SM,
  newdata = expand_grid(
    year = dat_sec_ab_be_SM$year %>% unique(),
    treatment = dat_sec_ab_be_SM$treatment %>% unique(),
    size_class = c("<50", "50-100"),
    habitat = dat_sec_ab_be_SM$habitat %>% unique(),
    log_depth = mean(dat_sec_ab_be_SM$depth) %>% 
      log(),
    log_n_frame_total = mean(dat_sec_ab_be_SM$n_frame_total) %>%
      log()
  ),
  re_formula = NA
)
...
```{r}
epred_sec_ab_be_SM <-  epred_draws(
  fit_sec_ab_be_SM,
  newdata = expand_grid(
    year = dat_sec_ab_be$year %>% unique(),
    treatment = dat_sec_ab_be_SM$treatment %>% unique(),
    size_class = c("<50", "50-100"),
    habitat = dat_sec_ab_be_SM$habitat %>% unique(),
    log_depth = mean(dat_sec_ab_be_SM$depth) %>% 
...
```{r}
epred_sec_ab_be_SM_effect_treat_year <- epred_sec_ab_be_SM %>%
  ungroup() %>%
  dplyr::select(year,
                treatment,
                .draw,
...
```{r}
epred_sec_ab_be_SM_effect_treat_year %>% 
...
```{r}
epred_sec_ab_be_SM_effect_year <- epred_sec_ab_be_SM %>%
  ungroup() %>% 
...
```{r}
epred_sec_ab_be_SM_effect_year %>% 
...
```{r}
epred_sec_ab_be_SM_effect_treat <- epred_sec_ab_be_SM %>%
  ungroup() %>%
  dplyr::select(year,
                treatment,
                .draw,
...
```{r}
epred_sec_ab_be_SM_effect_treat %>% 
  ggplot(aes(x = diff, y = size_class)) +
  geom_vline(xintercept = 0, linetype = "11", col = "grey")+
  stat_slab(alpha = 0.3, aes(fill = factor(size_class))) +
  stat_slab(show.legend = F,size = .5,  fill = NA ,  aes(col = factor(size_class))) +
...
```{r}
epred_sec_ab_be_SM_depth <-  epred_draws(
  fit_sec_ab_be_SM,
  newdata = expand_grid(
    year = dat_sec_ab_be_SM$year %>% unique(),
    treatment = dat_sec_ab_be_SM$treatment %>% unique(),
    ...
    habitat = dat_sec_ab_be_SM$habitat %>% unique(),
    log_depth = seq(
      min(dat_sec_ab_be_SM$log_depth),
      max(dat_sec_ab_be_SM$log_depth),
      length = 100
    ),
    log_n_frame_total = mean(dat_sec_ab_be$n_frame_total) %>%
      log()
  ),
  re_formula = NA
)

epred_sec_ab_be_SM_depthS <- epred_sec_ab_be_SM_depth %>% 
  group_by(log_depth) %>% 
...
```{r}
epred_sec_ab_be_SM_nframe <-  epred_draws(
  fit_sec_ab_be_SM,
  newdata = expand_grid(
    year = dat_sec_ab_be_SM$year %>% unique(),
    treatment = dat_sec_ab_be_SM$treatment %>% unique(),
    ...
    log_n_frame_total = seq(
      min(dat_sec_ab_be_SM$log_n_frame_total),
      max(dat_sec_ab_be_SM$log_n_frame_total),
      length = 100
    )),
  re_formula = NA
)

epred_sec_ab_be_SM_nframeS <- epred_sec_ab_be_SM_nframe %>% 
...
fit_sec_ab_be_L <- brm(
  ...
  seed = 123,
  file_refit = "on_change",
  file = here("models/fit_sec_ab_be_L"),
  threads = threading(2, static = T),
  backend = "cmdstanr"
)
```
...
```{r}
summary(fit_sec_ab_be_L)#ok
pp_check(fit_sec_ab_be_L, ndraws = 100)
bayes_R2(fit_sec_ab_be_L)#0.4
```

```{r}
epred_sec_ab_be_L <-  epred_draws(
  fit_sec_ab_be_L,
  newdata = expand_grid(
    year = dat_sec_ab_be_L$year %>% unique(),
    treatment = dat_sec_ab_be_L$treatment %>% unique(),
    habitat = dat_sec_ab_be_L$habitat %>% unique(),
    log_depth = mean(dat_sec_ab_be_L$depth) %>%
      log(),
    log_n_frame_total = mean(dat_sec_ab_be_L$n_frame_total) %>%
      log()
  ),
  re_formula = NA
) %>%
