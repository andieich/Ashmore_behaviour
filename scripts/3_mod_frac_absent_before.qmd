---
title: "Model behaviour fractions absent <> before"
author: "Andreas Eich"
format: html
editor: visual
...
```{r}
dat_frac_ab_be <- readRDS(here("data/clean/dat_absent_before.rds"))
```
...
```{r}
dat_frac_ab_beW <- dat_frac_ab_be %>%
...
dat_frac_ab_beL <- dat_frac_ab_beW %>%
...
Only 11 videos. Verified with original data:

-   2004: one video without fish observations (`ASH1_1`)

-   2016: one video only treatment = absent, i.e. not in this df

    **CHECK!!! More video missing??**
...
```{r}
dat_model_frac_ab_be <- dat_frac_ab_beW %>% 
...
```{r}
dat_model_frac_ab_be <- dat_model_frac_ab_be %>% 
...
```{r}
dat_model_frac_ab_be_SM <- dat_model_frac_ab_be %>% 
  filter(size_class != ">100") %>% 
  droplevels()

dat_model_frac_ab_be_L <- dat_model_frac_ab_be %>% 
  filter(size_class == ">100") %>% 
  droplevels()
```
...
```{r}
fit_frac_ab_be_SM <- brm(bf(cbind(frac_circ_invT, frac_passingT, frac_feedingT) ~ 
           ...
           file = here("models/fit_frac_ab_be_SM"),
           backend = "cmdstanr")
```

```{r}
summary(fit_frac_ab_be_SM)# 4 divergent transitions but Rhat ok
#plot(fit_frac_ab_be_SM) # ok
```

Get R2 value per category

```{r}
#dirichlet_r2(fit_frac_ab_be_SM)#takes a while
```

frac_circ_invT 0.3116627\
frac_passingT 0.3431775\
frac_feedingT 0.1187646

Get expected values

```{r}
epred_frac_ab_be_SM <-  epred_draws(
  fit_frac_ab_be_SM,
  newdata = expand_grid(
    year = dat_model_frac_ab_be_SM$year %>% unique(),
    treatment = dat_model_frac_ab_be_SM$treatment %>% unique(),
    size_class = c("<50", "50-100"),
    habitat = dat_model_frac_ab_be_SM$habitat %>% unique(),
    log_depth = mean(dat_model_frac_ab_be_SM$depth) %>% 
      log(),
    log_n_frame_total = mean(dat_model_frac_ab_be_SM$n_frame_total) %>%
      log()
  ),
  re_formula = NA
)
...
No difference when comparing between year over for combined data (absent & before), but differences in 2016 *between* absent and before.
...
```{r}
epred_frac_ab_be_SM_effect <- epred_frac_ab_be_SM %>%
  ungroup() %>% 
...
```{r}
epred_frac_ab_be_SM_depth <-  epred_draws(
  fit_frac_ab_be_SM,
  newdata = expand_grid(
    year = dat_model_frac_ab_be_SM$year %>% unique(),
    treatment = dat_model_frac_ab_be_SM$treatment %>% unique(),
    ...
    habitat = dat_model_frac_ab_be_SM$habitat %>% unique(),
    log_depth = seq(
      min(dat_model_frac_ab_be_SM$log_depth),
      max(dat_model_frac_ab_be_SM$log_depth),
      length = 100
    ),
    log_n_frame_total = mean(dat_model_frac_ab_be_SM$n_frame_total) %>%
      log()
...
epred_frac_ab_be_SM_depthS <- epred_frac_ab_be_SM_depth %>% 
...
epred_frac_ab_be_SM_depthS %>%
  ...
  geom_line(aes(y = median*100, col = .category), linewidth = 1, show.legend = F)+
   geom_point(data = dat_frac_ab_beL %>% 
               filter(size_class != ">100") %>% 
...
```{r}
epred_frac_ab_be_SM_depth_ntot <-  epred_draws(
  fit_frac_ab_be_SM,
  newdata = expand_grid(
    year = dat_model_frac_ab_be_SM$year %>% unique(),
    treatment = dat_model_frac_ab_be_SM$treatment %>% unique(),
    ...
    log_n_frame_total = seq(
      min(dat_model_frac_ab_be_SM$log_n_frame_total),
      max(dat_model_frac_ab_be_SM$log_n_frame_total),
      length = 100
    )),
  re_formula = NA
)
...
epred_frac_ab_be_SM_depth_ntotS <- epred_frac_ab_be_SM_depth_ntot %>% 
...
epred_frac_ab_be_SM_depth_ntotS %>%
...
A less complex model has to be used because number of observations low

```{r}
fit_frac_ab_be_L <- brm(
  ...
  file_refit = "on_change",
  threads = threading(2, static = T),
  file = here("models/fit_frac_ab_be_L"),
  backend = "cmdstanr"
)
```

```{r}
summary(fit_frac_ab_be_L)# 5 divergent transitions, Rhat ok
#plot(fit_frac_ab_be_L) # OK
```

```{r}
#irichlet_r2(fit_frac_ab_be_L)
```

frac_circ_invT 0.10770538\
frac_passingT 0.13704968\
frac_feedingT 0.03291023

Get expected values

```{r}
epred_frac_ab_be_L <-  epred_draws(
  fit_frac_ab_be_L,
  newdata = expand_grid(
    year = dat_model_frac_ab_be_L$year %>% unique(),
    treatment = dat_model_frac_ab_be_L$treatment %>% unique(),
    habitat = dat_model_frac_ab_be_L$habitat %>% unique(),
    log_depth = mean(dat_model_frac_ab_be_L$depth) %>% 
      log(),
    log_n_frame_total = mean(dat_model_frac_ab_be_L$n_frame_total) %>%
      log()
  ),
  re_formula = NA
) %>%
...
```{r}
epred_frac_ab_be_L_effect <- epred_frac_ab_be_L %>%
  ungroup() %>%
  dplyr::select(year,
                treatment,
                .draw,
