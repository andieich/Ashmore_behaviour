...
```{r}
dat_fsec_be_af <- readRDS(here("data/clean/dat_before_after.rds")) 

dat_fsec_be_af <- dat_fsec_be_af %>% 
...
```{r}
hist(dat_fsec_be_af$feeding_time_sec, breaks = 40)
```

Try zero-inflated lognormal model

```{r}
dat_fsec_be_af_S <- dat_fsec_be_af %>%
  filter(size_class == "<50")


dat_fsec_be_af_M <- dat_fsec_be_af %>%
  filter(size_class == "50-100")



fit_fsec_be_af_S <- brm(
  bf(
  ...
  file = here("models/fit_fsec_be_af_S"),
  backend = "cmdstanr"
)

fit_fsec_be_af_M <- brm(
  bf(
    feeding_time_sec ~ factor(year) * treatment +
      s(log_n_frame_total) + 
                  habitat + 
  ...
  file_refit = "on_change",
  threads = threading(2, static = T),
  file = here("models/fit_fsec_be_af_M"),
  backend = "cmdstanr"
)
...
```{r}
summary(fit_fsec_be_af_S)#ok

bayesplot::ppc_dens_overlay(y = log1p(dat_fsec_be_af_S$feeding_time_sec), 
                            yrep = log1p(posterior_predict(fit_fsec_be_af_S)[1:10,]))

bayes_R2(fit_fsec_be_af_S)#0.12
```

test: only medium sized fish, 2004

```{r}

fit_fsec_be_af_M_04 <- brm(
  bf(
    feeding_time_sec ~ treatment +
                  (1|video), 
    hu ~ treatment,
    decomp = "QR"
  ),
  ...
  file_refit = "on_change",
  threads = threading(2, static = T),
  file = here("models/fit_fsec_be_af_M_04"),
  backend = "cmdstanr"
)
```

```{r}
summary(fit_fsec_be_af_M_04)#ok

bayesplot::ppc_dens_overlay(y = log1p(dat_fsec_be_af_M$feeding_time_sec), 
                            yrep = log1p(posterior_predict(fit_fsec_be_af_M_04)[1:10,]))

bayes_R2(fit_fsec_be_af_M_04)#0.4
```

```{r}
cols_treat <- c("#FFDB6D", "#4E84C4")

dat_fsec_be_af_M_04 <- dat_fsec_be_af_M %>% 
  filter(year == 2004)

epred_fsec_be_af_M_04 <-  epred_draws(
  fit_fsec_be_af_M_04,
  newdata = expand_grid(
    treatment = dat_fsec_be_af_M_04$treatment %>% unique(),
    habitat = dat_fsec_be_af_M_04$habitat %>% unique(),
    log_depth = mean(dat_fsec_be_af_M_04$depth) %>%
      log(),
    log_n_frame_total = mean(dat_fsec_be_af_M_04$n_frame_total) %>%
      log()
  ),
  ,
  re_formula = NA
...
epred_fsec_be_af_M <- epred_fsec_be_af_M %>% 
  filter(.epred != Inf)
```
